from lexical_analysis import lexical
import tkinter
...
def on_click():
    ...
    output, symbols = lexical(path)
    for key, value in symbols.items():
        output_1 += key + '\t' + str(value) + '\n'

    text.insert(1.0, output)
    text_1.insert(1.0, output_1)
...
if __name__=='__main__':
    ...
    tkinter.Button(fram, text='词法分析', command=on_click, font=("Arial", 12), width=7, height=1).pack(side=tkinter.TOP)
    fram.pack(side=tkinter.LEFT)

    fram1 = tkinter.Frame(root)
    tkinter.Label(fram1, text='token序列', font=("Arial", 12), width=7, height=2).pack(side=tkinter.TOP)
