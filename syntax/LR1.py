	...
	def calculate_first(self):
		...
		while done:
			...
			for i, grammar in enumerate(self.grammars):
				...
				# if grammar[1] in self.Vt:
				# 	if grammar[1] not in first[grammar[0]]:
				# 		done = True
				# 		first[grammar[0]].append(grammar[1])
				# elif grammar[1] in self.Vn:
				# 	if grammar[1] in first.keys():
				# 		if len(first[grammar[1]]) > 0:
				# 			for symbol in first[grammar[1]]:
				# 				if symbol not in first[grammar[0]]:
				# 					done = True
				# 					first[grammar[0]].append(symbol)
